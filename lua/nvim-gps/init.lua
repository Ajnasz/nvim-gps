...
local update_tree = ts_utils.memoize_by_buf_tick(function(bufnr)
    local filelang = ts_parsers.ft_to_lang(api.nvim_buf_get_option(bufnr, "filetype"))
    local parser = ts.get_parser(bufnr, filelang)
    return parser:parse()
end)
...
function M.get_data()
    ...
    local curr_loc = api.nvim_win_get_cursor(0)
    if data_prev_loc[1] == curr_loc[1] and data_prev_loc[2] == curr_loc[2] then
        return data_cache_value
    end

    data_prev_loc = api.nvim_win_get_cursor(0)

    local filelang = ts_parsers.ft_to_lang(vim.bo.filetype)
    local gps_query = ts.query.get(filelang, "nvimGPS")
    ...
    update_tree(api.nvim_get_current_buf())
    ...
    local function add_node_data(pos, capture_name, capture_node)
        ...
        if vim.fn.has("nvim-0.7") > 0 then
            text = ts.get_node_text(capture_node, 0)
            if text == nil then
                return data_cache_value
            end
...
function M.get_location(opts)
    ...
    location_prev_loc = api.nvim_win_get_cursor(0)
